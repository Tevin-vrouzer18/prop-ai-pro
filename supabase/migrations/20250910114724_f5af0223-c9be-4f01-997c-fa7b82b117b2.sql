-- Ensure clean state for tenant_info policies
...
-- Recreate explicit tenant_info policies
...
-- Ensure leases policies exist (drop and recreate)
DROP POLICY IF EXISTS "Landlords can insert leases for their units" ON public.leases;
DROP POLICY IF EXISTS "Landlords can update leases for their units" ON public.leases;
